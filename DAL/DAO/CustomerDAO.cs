...
namespace DAL.DAO
{
    public class CustomerDAO : DbContext
    {
        ...
        public static List<customer> Search(string text)
        {
            try
            {
                List<customer> list = db.customers.Where(x => x.customer_id.Contains(text) || x.address.Contains(text) || x.contact.Contains(text) || x.dob.ToString().Contains(text) || x.name.Contains(text)).ToList();
                return list;
            }
            catch (Exception e)
            {
