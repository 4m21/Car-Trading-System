...
namespace DAL.DAO
{
    public class OrderDAO : DbContext
    {
        ...
        public static List<order> GetMyOrders(string customerID)
        {
            return db.orders.Where(x => x.customer_id == customerID).ToList();
        }
        ...
        public static List<order> Search(string text)
        {
            try
            {
                List<order> list = db.orders.Where(x => x.order_id.ToString() == text || x.customer_id == text || x.order_desc.Contains(text)).ToList();
                return list;
            }
            catch (Exception e)
            {
